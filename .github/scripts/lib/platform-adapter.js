 ...
 * @param {PlatformType} [platform] - Platform type (auto-detected if not provided)
 * @returns {Promise<PlatformAdapter>}
 */
export async function createPlatformAdapter(platform) {
  const detectedPlatform = platform || detectPlatform();

  if (detectedPlatform === 'bitbucket') {
    const { BitbucketAdapter } = await import('./bitbucket-adapter.js');
    return BitbucketAdapter.create();
  } else {
    const { GitHubAdapter } = await import('./github-adapter.js');
    return GitHubAdapter.create();
  }
}
